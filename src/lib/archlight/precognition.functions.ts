...
async function fetchYahooSeries(symbol: string): Promise<MarketPoint[]> {
  ...
  const res = await fetch(url, { headers: { "User-Agent": "Archlight/1.0" } });
  if (!res.ok) return [];
