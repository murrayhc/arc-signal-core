  ...
  const result = await runLLMTask(
    ...
    },
    {
      ...
      validate: {
        ...
        // Centralised guaranteed-outcome rejection: runs over the raw text inside
        // runLLMTask's own validation, so a match makes the run REJECTED_VALIDATION
        // (redacted outputSummary) rather than a SUCCEEDED run whose text is
        // discarded post-hoc by this caller — see findGuaranteedOutcomeLanguage.
        extraCheckers: [findGuaranteedOutcomeLanguage],
      },
    },
  )
